...
#+BEGIN_SRC emacs-lisp
(defvar org-tree-view-mode-map (make-keymap))
...
`org-tree-view-headline-pos' text property."
  (let* ((title (org-element-property :raw-value headline))
    ...
    (put-text-property ; org-tree-view-level-N
     ...
     text)
    (setq text (org-tree-view/draw-string-like-org text))
...
(defun org-tree-view/draw-string-like-org (text)
  "Add, as Org would, proper faces for italic, bold and verbatim
text. Regardless of Org settings,
remove the markup for said
types. Return the text with the new properties."
  (let* ((text (concat text " ")) ; this is needed in case the string ends with one of the `types'
         (types-faces
          '(italic italic bold bold verbatim org-verbatim))
         (types (--filter it
                          (-map-indexed
                           (lambda (i x) (if (= (% i 2) 0) x)) types-faces)))
         (data (org-element-parse-secondary-string text types)))
    (org-element-map data types
      (lambda (object)
        (let* ((type (org-element-type object))
               (value (org-element-property :raw-value object))
               (begin (1- (org-element-property :begin object)))
               (end (1- (org-element-property :end object)))
               (face (plist-get types-faces type)))
          (message "%s" type)
          (put-text-property ; add face for type
           begin end
           'face `(,face ,(get-text-property begin 'face text))
           text)
          ...
          (put-text-property ; remove second piece of markup
           (- end 2) (if (= end (length text)) end (1- end))
           'display ""
           text))))
    text))
...
(defun org-tree-view/switch-to-base-buffer ()
  ...
  (when (org-tree-view/is-tree-view)
    (if (get-buffer-window (org-tree-view/make-base-buffer-name))
      ...
      (other-window 1)
      (switch-to-buffer (org-tree-view/make-base-buffer-name) nil :force-same-window)))) ; this last part doesn't work?
...
(defun org-tree-view/open (&optional arg)
  ...
  (let ((tree-view-buffer-name (org-tree-view/make-tree-view-buffer-name))
    ...
    ;; (org-mode)
    ;; (org-mode)
    ;; (setq-local org-indent-mode nil) ; move to org-tree-view-mode?
    ;; (outline-show-all)
    (setq-local buffer-read-only t)
    (org-tree-view-mode)))
