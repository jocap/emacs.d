...
work - and nicely, at that! The following code exists in addition to some
NeXTSTEP-specific stuff in =custom.el=.

*** Fonts

#+NAME: c:osx
#+BEGIN_SRC emacs-lisp
(defun osx-theme-do-all (&optional r)
  (when (eq system-type 'darwin)
    ;; Replace Sans Serif with Lucida Grande
    (mapc
     (lambda (face)
       (when (string-equal (face-attribute face :family) "Sans Serif")
         (set-face-attribute face nil :family "Lucida Grande")))
     (face-list))))
(advice-add #'theme-do-all :after #'osx-theme-do-all)

(defalias 'x-selection-owner-p 'ns-selection-owner-p)
#+END_SRC

Note to self: the above method of changing fonts is rather flexible and probably
suitable for the configuration of other operating systems as well.
