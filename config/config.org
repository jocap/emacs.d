...
# TODO:
# - Replace (kill-buffer) + (delete-window) with (quit-window)
...
(defcustom org-tree-view/key-binding "C-c C-t"
...
#+NAME: c:org-tree-view
#+BEGIN_SRC emacs-lisp
(defun org-tree-view/open-heading (&optional no-bind-exit)
  ...
  (let ((tree-view-buffer-name (org-tree-view/buffer-name)))
    (when (get-buffer-window tree-view-buffer-name)
      (with-current-buffer tree-view-buffer-name
        (outline-show-all)
        ;; Show appropriate levels of headings
        (let ((current-prefix-arg
               (if (and (boundp 'org-tree-view/show-all) org-tree-view/show-all)
                   99 ; show all
                 org-tree-view/levels)) ; default
              (inhibit-message t))
...
#+NAME: c:org-tree-view
#+BEGIN_SRC emacs-lisp
(defun org-tree-view/toggle-show-all ()
  "Toggle between showing all headings and the levels of headings
set in `org-tree-view/levels'."
  (interactive)
  (if (and (boundp 'org-tree-view/show-all) org-tree-view/show-all)
      (setq-local org-tree-view/show-all nil)
    (setq-local org-tree-view/show-all t))
  (org-tree-view/refresh))
#+END_SRC
...
(defun org-tree-view/buffer-name ()
  "Return the appropriate name for the current file's tree view buffer."
...
#+NAME: c:org-tree-view
#+BEGIN_SRC emacs-lisp
(defun org-tree-view/base-buffer-name ()
  "Return the appropriate base buffer name for the tree view."
  (s-chop-prefix "<tree>" (buffer-name)))
#+END_SRC
...
(defun org-tree-view/isearch-return ()
  ...
  (when (s-starts-with? "<tree>" (buffer-name))
    (org-tree-view/open-heading :no-bind-exit))
  (run-with-timer 0 nil (lambda () (org-tree-view/bind-exit (kbd "<S-return>"))))
...
(defun org-tree-view/exit ()
  ...
  (let* ((tree-view-buffer-name (org-tree-view/buffer-name))
         ...
         (base-window (get-buffer-window (org-tree-view/base-buffer-name))))
    (kill-buffer tree-view-buffer-name)
...
#+NAME: c:org-tree-view
#+BEGIN_SRC emacs-lisp
(defun org-tree-view/bind-exit (&optional key)
  (let* ((key (or key (this-command-keys-vector)))
         ...
         (base-buffer (org-tree-view/base-buffer-name)))
    (unless (s-contains? "mouse" (key-description key))
...
This function binds =(this-command-keys-vector)= (or, if provided, =key=) to
=org-tree-view/exit= for 1 second, after which it returns the key binding to its
original function. Flexible!
...
buffer."
  (interactive)
  (require 's)
  (unless (eq (mark) (point)) (push-mark)) ; in case user gets lost
  ...
  (if (buffer-base-buffer)
    ...
    (let ((tree-view-buffer-name (org-tree-view/buffer-name)))
      (if (get-buffer-window tree-view-buffer-name)
        ...
        ;; The above code replaces the ellipsis with a string of spaces as long
        ;; as org-tree-view/width * 2. If Org headings are themed with a
        ;; background, this creates a nice, unified look.
        ...
        ;; Show all headings
        (local-set-key (kbd "<tab>") #'org-tree-view/toggle-show-all)

        ;; Quit tree view
        (local-set-key (kbd "Q") #'org-tree-view/exit)
        (local-set-key (kbd "C-g") #'org-tree-view/exit)
        ...
        (local-set-key (kbd "<S-return>") #'org-tree-view/open-heading)
...
#+NAME: c:org-tree-view
#+BEGIN_SRC emacs-lisp
(defvar org-tree-view/show-all nil
  "Local variable telling org-tree-view/refresh to show all levels.")
#+END_SRC
...
When exiting the tree view for an Org document in the right window, the original
width of the right window will be shrunk.
...
When searching for the last word in a heading, you only see the following:

#+CAPTION: Searching for "configuration", matching a heading ending with "configuration"
#+NAME: fig:org-tree-view:bug:searching
[[./resources/org-tree-view-bug.png]]

I'm not quite sure how to fix it yet. It could possibly be fixed by appending
some searchable text (I'm thinking spaces) to every heading, but which isn't
actual text inserted and saved into the buffer. Perhaps this could be achieved
using text properties or maybe overlays.

To alleviate the issue in the meantime, you can press ~C-r~ whenever this happens
and you will see the the word you're searching for.

Actually, that gives me an idea - a simple solution would be to advice
=isearch-repeat-forward= (and =isearch-forward=, to be sure) to automatically run
=iesarch-repeat-backward= after it's done. Or perhaps there's a way to force
=isearch= to always put the point at the beginning of the match?
