...
(setq nsm-settings-file (state-dir "network-security.data"))
...
view, but only for 1 second."
  (let* ((key (or key (this-command-keys-vector)))
    ...
    (when (and base-buffer (not (s-contains? "mouse" (key-description key))))
...
For my Gnus configuration, see the =config/gnus.el= file.
