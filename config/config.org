...
*** /org-tree-view/ [[#org-tree-view][Test ~code~ and =verbatim= link]]
:PROPERTIES:
...
types. Return the text with the new properties."
  (let* ((types '(link
    ...
    (org-element-map data types
      (org-tree-view/propertize-fun text types))
    text))

(defun org-tree-view/propertize-fun (text types)
  (lambda (object)
    (let* ((type (org-element-type object))
           ...
           (faces
            '(italic italic bold bold verbatim org-verbatim code org-code)))
