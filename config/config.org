...
***** /org-tree-view/start-search/

#+NAME: c:org-tree-view
#+BEGIN_SRC emacs-lisp
(defun org-tree-view/start-search (N)
  (interactive "p")
...
(defun org-tree-view/self-insert-command (oldfun N)
  (interactive "p")
  (if (s-starts-with? "<tree>" (buffer-name))
      (call-interactively #'org-tree-view/start-search)
    (call-interactively oldfun)))
#+END_SRC
...
actual text inserted and saved into the buffer. This doesn't seem to be possible
using text properties or overlays, as those aren't searchable. The issue really
lies with =isearch=.

A possible solution could be advising =isearch-search-and-update= to scroll the
screen to the left after its done.
